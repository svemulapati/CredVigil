...
flowchart TB
    ...
    subgraph SCANNER["GitScanner"]
        ...
        S5["Run detection engine<br/>(369 rules + entropy)"]
...
flowchart TB
    ...
    subgraph C1["Component 1: Detection Engine"]
        ENGINE["ScanContent<br/>369 rules + entropy"]
    end
...
It uses the same detection engine from Module 1 — all 369 rules plus entropy detection. The scanner simply feeds the added lines from each diff entry into `engine.ScanContent()`. No git-specific rules are needed because the content is just text, regardless of where it came from.

```mermaid
flowchart LR
    GIT["Added lines from git diff"] --> BUILD["Build content string"]
    BUILD --> ENGINE["Same 369-rule engine<br/>from Module 1"]
...
4. **Integration with the Detection Engine** — Feeding changed files into the same 369-rule engine
